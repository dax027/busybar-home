...
the corresponding stock BUSY Bar animations; In a meeting, Stepped away, Hacking, and Daydreaming
use custom device-native animations generated by this project. Playback happens on the device, so
the app does not simulate animation with a continuous network-command loop.
