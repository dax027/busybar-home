...
| --- | --- | --- |
| Deep focus | Animated work checklist with large `BUSY` | `DEEP FOCUS` — One task. No inbox. |
| Focused | Distractions collapsing into a cyan-violet `FOCUSED` lock | `FLOW STATE` — Hold the thread. |
| Concentrating | A flashing PET-scan brain firing into a scrolling `CONCENTRATING` | `DEEP WORK` — Working it through. |
| In a meeting | Animated live microphone with `ON A CALL` | `MEETING MODE` — Capture decisions. |
| Available | Mint-green neon `FREE` sign | `RESET WINDOW` — Clear quick replies. |
| I love my wife | Heartbeat-driven positive/negative `I ♥ MY WIFE` | `LUCKY HUSBAND` — Tell her. Show her. |
| Stepped away | Ticking analog clock with large `BRB` | `SHORT BREAK` — Pause. Reset. Return. |
...
| Gaming | Pixel controller powering into a shifting `GAMING` wordmark | `PLAYER ONE` — Relax. Play. Recharge. |
| Hacking | Animated pixel mask reveal that glitches into `HACKING` | `CYBER OPS` — Map. Probe. Learn. |
| Daydreaming | Animated sky with drifting clouds behind `DAYDREAMING` | `WANDER MODE` — Let ideas drift. |

The browser previews use the same frames shown on the BUSY Bar. Coding and Low social battery use
the corresponding stock BUSY Bar animations; Deep focus, Focused, Concentrating, In a meeting,
Available, I love my wife, Stepped away, Gaming, Hacking, and Daydreaming use custom device-native
animations generated by this project. Playback happens on the device, so the app does not simulate
animation with a continuous network-command loop.
